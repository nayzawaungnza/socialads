@extends('layouts/master', ['activePage' => 'posts.update', 'titlePage' => __('Edit Post')])
...
@section('content')
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
            <h4 class="py-3 mb-4">
                <span class="text-muted fw-light">post/</span> Update
            </h4>
           ...
           <div class="card mb-4">
                ...
                @else
                  <form  class="card-body" method="POST" action="{{url('/admin/posts/'.$post->id.'')}}" enctype="multipart/form-data" >
                @endisset
                  @csrf
                  @method('PUT')
                  ...
                  <div class="row g-3">
                    <div class="col-md-12">
                      ...
                      <input type="text" id="name" name="name" value="{{ old('name', $post->name) }}" class="form-control @error('name') is-invalid @enderror" placeholder="Enter Post Name" />
                      @error('name')
                    ...
                    <div>
                      ...
                      <textarea id="editor" name="description" style="height: 300px; width: 100%;" class=" form-control" placeholder="Enter Description">{{ old('description', $post->description) }}</textarea>
    ...
    @if(!empty($categories))
        @foreach($categories as $category)
            <div class="form-check">
                <input 
                    class="form-check-input @error('post_category') is-invalid @enderror" 
                    type="checkbox" 
                    id="category_{{ $category->id }}" 
                    name="post_category[]" 
                    value="{{ $category->id }}"
                    {{ (is_array(old('post_category', isset($post) ? $post->postCategories->pluck('id')->toArray() : [])) && in_array($category->id, old('post_category', isset($post) ? $post->postCategories->pluck('id')->toArray() : []))) ? 'checked' : '' }}
                >
                ...
                @foreach($category->children as $child)
                    <div class="form-check ms-3">
                        <input 
                            ...
                            name="post_category[]" 
                            value="{{ $child->id }}"
                            {{ (is_array(old('post_category', isset($post) ? $post->postCategories->pluck('id')->toArray() : [])) && in_array($child->id, old('post_category', isset($post) ? $post->postCategories->pluck('id')->toArray() : []))) ? 'checked' : '' }}
                        >
                        ...
                        @foreach($child->children as $subChild)
                            <div class="form-check ms-5">
                                <input 
                                    ...
                                    name="post_category[]" 
                                    value="{{ $subChild->id }}"
                                    {{ (is_array(old('post_category', isset($post) ? $post->postCategories->pluck('id')->toArray() : [])) && in_array($subChild->id, old('post_category', isset($post) ? $post->postCategories->pluck('id')->toArray() : []))) ? 'checked' : '' }}
                                >
                  ...
                  <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        @enderror
                         ...
                         <div class="mt-3">
                            <img id="imagePreview" 
                                src="{{ isset($post) && $post->default_image ? asset('storage/' . $post->default_image->image_url)  : asset('assets/img/backgrounds/5.jpg') }}" 
                                alt="Preview Image" 
                    ...
                    <div class="col-md-6">
                      ...
                      <textarea id="excerpt" value="{{ old('excerpt', $post->excerpt) }}" name="excerpt" class="form-control @error('excerpt') is-invalid @enderror" placeholder="enter excerpt"></textarea>
                      @error('excerpt')
                          <span class="error invalid-feedback" style="margin-left:8px;display:block;">{{ $message }}</span>
                        @enderror
                    </div>
