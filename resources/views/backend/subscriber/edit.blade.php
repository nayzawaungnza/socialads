@extends('layouts/master', ['activePage' => 'subscribers.edit', 'titlePage' => __('Edit Subscriber Account')])
...
<h4 class="py-3 mb-4">
 <span class="text-muted fw-light">Subscribers / Edit /</span> Account
</h4>
...
<div class="row">
  <div class="col-md-12">
    ...
    <div class="card mb-4">
      <h5 class="card-header">Subscribers Profile Update</h5>
      <!-- Account -->
      @isset($url)
      <form method="POST" class="url" action="{{$url}}" enctype="multipart/form-data">
      @else
      <form method="POST" action="{{url('/admin/subscribers/'.$subscriber->id)}}" enctype="multipart/form-data">
      @endisset
      @csrf
      @method('PUT')
        ...
        <div class="card-body">
          <div class="d-flex align-items-start align-items-sm-center gap-4">
            <img src="{{ $subscriber->default_image ? $subscriber->default_image?->user_default_image_url : url('storage/images/profile-picture.png') }}" alt="user-avatar" class="d-block w-px-100 h-px-100 rounded" id="uploadedAvatar" />
        ...
        <div class="card-body">
          <div class="row g-3">
                      <div class="col-md-6">
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name', $subscriber->name) }}" class="form-control @error('name') is-invalid @enderror" placeholder="john.doe" />
                        @error('name')
                          <span class="error invalid-feedback" style="margin-left:8px;display:block;">{{ $message }}</span>
                        @enderror
                      </div>
                    ...
                    <div class="row g-3">
                      ...
                      <div class="col-md-6">
                        ...
                        <input type="text" id="multicol-address" value="{{ old('address', $subscriber->address) }}" name="address" class="form-control @error('address') is-invalid @enderror" placeholder="address" />
                        @error('address')
                            <span class="error invalid-feedback" style="margin-left:8px;display:block;">{{ $message }}</span>
                          @enderror
                      </div>
