    ...
    <div class="modal-content p-3 p-md-5">
      <div class="modal-body">
        ...
        <div class="text-center mb-4">
          ...
          <p class="text-muted">Updating student details will receive a privacy audit.</p>
        </div>
        <form id="editUserForm" class="row g-3" action="{{ route('teachers.update', $student->id) }}" method="POST" onsubmit="return false">
          <div class="col-12 col-md-6">
            <label class="form-label" for="modalEditUserFirstName">Name</label>
            <input type="text" id="modalEditUserFirstName" name="name" value="{{ old('name', $student->name) }}" class="form-control @error('name') is-invalid @enderror" placeholder="John" />
            @error('name')
          ...
          <div class="col-12 col-md-6">
            ...
            <input type="text" id="modalEditUserEmail" value="{{ old('email', $student->email) }}" name="email" class="form-control @error('email') is-invalid @enderror" placeholder="example@domain.com" />
          </div>
           <div class="col-12 col-md-6">
            <label class="form-label" for="modalEditUserPhone">Phone Number</label>
            <div class="input-group">
              {{-- <span class="input-group-text">MM (+1)</span> --}}
              <input type="text" id="modalEditUserPhone" name="mobile" value="{{ old('mobile', $student->mobile) }}" class="form-control phone-number-mask @error('mobile') is-invalid @enderror" placeholder="09 *** *** ***" />
            </div>
          </div>
          <div class="col-12 col-md-6">
                              ...
                              <div class="row">
                                  <div class="col-md-3 select2-purple">
                                      <select class="nrc_state form-control @error('nrc_number') is-invalid @enderror select2"  name="nrc_state" data-placeholder="xx" data-dropdown-css-class="" style="width: 100%;" data-attr-url="{{url('admin/nrc/getNrcTownshipByStateId')}}">
                                          ...
                                          <option value="{{ $state->id }}" {{ ($student->nrc && $state->code==explode("-",$student->nrc)[0])?"selected":"" }}>{{$state->code}}</option>
                                          @endforeach
                                      </select>
                                      @error('nrc_state')
                                        <span class="error invalid-feedback" style="margin-left:8px;display:block;">{{ $message }}</span>
                                  ...
                                  <div class="col-md-4 select2-purple">
                                      <select class="nrc_township form-control @error('nrc_number') is-invalid @enderror select2" name="nrc_township" data-placeholder="xxx">
                                          ...
                                          <option value="{{ $township->id }}" {{ ($student->nrc && $township->code==explode("-",$student->nrc)[1])?"selected":"" }}>{{$township->code}}</option>
                                          @endforeach
                                      </select>
                                      @error('nrc_township')
                                        <span class="error invalid-feedback" style="margin-left:8px;display:block;">{{ $message }}</span>
                                  ...
                                  <div class="col-md-2 select2-purple">
                                      <select class="nrc_type form-control @error('nrc_number') is-invalid @enderror select2" name="nrc_type" data-placeholder="x">
                                          ...
                                          <option value="{{ $type->id }}" {{ ($student->nrc && $type->code==explode("-",$student->nrc)[2])?"selected":"" }}>{{$type->code}}</option>
                                          @endforeach
                                      </select>
                                      @error('nrc_type')
                                        <span class="error invalid-feedback" style="margin-left:8px;display:block;">{{ $message }}</span>
                                  ...
                                  <div class="col-md-3 select2-purple">
                                      <input type="text" name="nrc_number" class="form-control @error('nrc_number') is-invalid @enderror" id="nrc_number" placeholder="000000" value="{{ old('nrc_number', $student->nrc ? explode("-",$student->nrc)[3] : '') }}">
