    ...
    <div class="container-fluid px-4 px-md-5 px-lg-5 px-xl-5 max-width-container mx-auto">
        <div class="row justify-content-center align-items-stretch gx-4 gy-4">
            ...
            <div class="col-12 col-lg-5">
                <h3 class="fs-4 fw-semibold text-dark-blue">Featured Posts</h3>
                <hr class="bg-teal my-2" style="height: 4px; width: 50px;">
                        ...
                        @foreach($posts as $post)
                            <div class="border-bottom py-3">
                                <a href="{{ route('website.post-detail', $post->slug) }}" 
                                   class="text-dark fw-semibold line-clamp-2 mb-2 d-block hover-underline">
                                    {{ $post->name }}
                                </a>
                                <div class="d-flex justify-content-between align-items-center">
                                    <a href="#" 
                                       class="tag text-primary fs-sm rounded-pill px-2 py-1 bg-gray-light hover-bg-gray">
                                        {{$post->createdBy?->name}}
                                    </a>
                                    <span class="fs-sm text-muted">{{ $post->created_at->format('d/m/Y') }}</span>
                                </div>
                            </div>
                        @endforeach
                     @endif
...
<div id="blog_three_column_01" class="container blog-three-column py-4">
    <div class="d-flex align-items-end mb-4 pb-3 border-bottom border-2 border-teal">
        <h2 class="fs-4 fw-semibold text-dark-blue m-0">Latest</h2>
        <a href="#" 
           class="ms-auto d-flex align-items-center text-dark fw-semibold hover-underline text-decoration-none">
            <span>View More</span>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="ms-1">
                <path d="M13.172 12L8.222 7.04999L9.636 5.63599L16 12L9.636 18.364L8.222 16.949L13.172 11.999L13.172 12Z" fill="#213343"/>
            </svg>
        </a>
    </div>
