...
$(function () {
  var maxlengthInput = $('.bootstrap-maxlength-example'),
    ...
    singleformRepeater = $('.single-form-repeater'),
    multipleformRepeater = $('.mutiple-form-repeater'),
    blankformRepeater = $('.blank-form-repeater');
  ...
  if (formRepeater.length) {
    var row = 2;
    var col = 1;
    formRepeater.on('click', function (e) {
      e.preventDefault();
    });
  ...
  if (singleformRepeater.length) {
    var row = 2;
    var col = 1;
    singleformRepeater.on('click', function (e) {
      e.preventDefault();
    });
    singleformRepeater.repeater({
      show: function () {
        var fromControl = $(this).find('.form-control, .form-select, .form-check-input');
        var formLabel = $(this).find('.form-label');

        fromControl.each(function (i) {
          var id = 'single-form-repeater-' + row + '-' + col;
          $(fromControl[i]).attr('id', id);
          $(formLabel[i]).attr('for', id);
          col++;
        });
  ...
  if (multipleformRepeater.length) {
    var row = 2;
    var col = 1;
    multipleformRepeater.on('click', function (e) {
      e.preventDefault();
    });
    multipleformRepeater.repeater({
      show: function () {
        var fromControl = $(this).find('.form-control, .form-select, .form-check-input');
        var formLabel = $(this).find('.form-label');

        fromControl.each(function (i) {
          var id = 'multiple-form-repeater-' + row + '-' + col;
          $(fromControl[i]).attr('id', id);
          $(formLabel[i]).attr('for', id);
          col++;
        });
  ...
  if (blankformRepeater.length) {
    var row = 2;
    var col = 1;
    blankformRepeater.on('click', function (e) {
      e.preventDefault();
    });
    blankformRepeater.repeater({
      show: function () {
        var fromControl = $(this).find('.form-control, .form-select, .form-check-input');
        var formLabel = $(this).find('.form-label');

        fromControl.each(function (i) {
          var id = 'blank-form-repeater-' + row + '-' + col;
          $(fromControl[i]).attr('id', id);
          $(formLabel[i]).attr('for', id);
          col++;
        });
