...
use App\Repositories\Backend\VideoRepository;
...
class VideoService implements VideoServiceInterface
{
    ...
    public function __construct(VideoRepository $imageRepository)
    {
        $this->videoRepository = $imageRepository;
    }

    public function getVideos()
    {
        if (request()->is('api/*')) {
            return $this->videoRepository->getImages();
        }
        return $this->videoRepository->orderBy('created_at', 'desc')->get();
    }
    ...
    public function update(Video $video, array $data)
    {
        DB::beginTransaction();
        try {
            $result = $this->videoRepository->update($video, $data);
        } catch (Exception $exc) {
            DB::rollBack();
            Log::error($exc->getMessage());
            throw new InvalidArgumentException('Unable to update video');
        }
        DB::commit();
    ...
    public function destroy(Video $video)
    {
        ...
        try {
            ...
            DB::rollBack();
            Log::error($exc->getMessage());
            throw new InvalidArgumentException('Unable to delete video');
        }
        DB::commit();
     ...
     * @return void
     */
    public function storeVideosFromZip(array $data, string $path, string $model_type, string $video_type = null)
    {
        try {
            $zip_file = $data['videos'];
            ...
            //dd('yes temp');
            $this->validateImages($temp_dir, $model_type, $video_type);
            ...
            Log::error($exc->getMessage());
            throw new InvalidArgumentException('Unable to store videos');
        }
    }
